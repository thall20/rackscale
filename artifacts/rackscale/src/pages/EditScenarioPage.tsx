...
import {
  getScenarioWithResult, updateScenario, createScenarioResult,
} from "@/lib/supabase-projects";
import { computeScenario } from "@/lib/calculations";
import { calculateScenario } from "@/lib/calculateScenario";
import { useCompanyPlan } from "@/hooks/useCompanyPlan";
import { canEditScenario, canUseFacilityConstraints } from "@/lib/featureAccess";
...
// ── Schema (identical to NewScenarioPage) ─────────────────────────────────────
...
// ── Map Scenario DB row → FormValues ─────────────────────────────────────────

function toFormValues(s: Scenario): FormValues {
  return {
    name: s.name,
    description: s.description ?? "",
    rackCount: s.rack_count,
    kwPerRack: s.avg_power_per_rack_kw,
    growthBufferPct: s.growth_buffer_pct,
    redundancyType: s.redundancy_level as "N" | "N+1" | "2N",
    utilityFeed: s.utility_feed,
    upsType: s.ups_type,
    coolingType: s.cooling_type,
    pueTarget: s.pue_target,
    containmentType: s.containment_type,
    ...
    flooringType: (s.flooring_type as FormValues["flooringType"]) ?? undefined,
...
export default function EditScenarioPage() {
  ...
  const { data: scenarioData, isLoading } = useQuery({
    ...
    queryFn: () => getScenarioWithResult(projectId, scenarioId),
    enabled: !!(projectId && scenarioId && companyId),
  });
  ...
  // Pre-populate form once the scenario loads
  useEffect(() => {
    if (scenarioData) {
      form.reset(toFormValues(scenarioData));
      setFacilityEnabled(scenarioData.facility_constraints_enabled && hasFacility);
    }
  }, [scenarioData, hasFacility]); // eslint-disable-line react-hooks/exhaustive-deps

  const mutation = useMutation({
    mutationFn: async (values: FormValues) => {
      const fcEnabled = facilityEnabled && hasFacility;

      // 1. Update scenario inputs
      ...
      // 2. Re-run core power / cost metrics
      ...
      // 3. Re-run facility constraints engine
      ...
      // 4. Upsert result (createScenarioResult uses onConflict: scenario_id)
    ...
    onSuccess: () => {
      ...
      setLocation(`/projects/${projectId}/scenarios/${scenarioId}`);
    },
    onError: (err: Error) => {
    ...
    return (
      <SidebarLayout>
        <div className="p-6 max-w-2xl mx-auto w-full space-y-6">
          <Link href={`/projects/${projectId}/scenarios/${scenarioId}`}>
            <Button variant="ghost" size="sm" className="-ml-3 text-muted-foreground">
              <ArrowLeft className="h-4 w-4 mr-2" /> Back to Scenario
            </Button>
          </Link>
          <div className="flex flex-col items-center text-center py-16 px-8 border-2 border-dashed rounded-2xl bg-muted/10">
            ...
            <p className="text-muted-foreground max-w-sm mb-8">
              Editing scenarios is available on Pro and higher. Upgrade to modify inputs and re-run your analysis.
            </p>
            <Link href="/billing">
              <Button>
        ...
        <div>
          ...
          {isLoading ? (
            ...
            <>
              ...
              <p className="text-muted-foreground">Update inputs and re-run the analysis.</p>
            </>
          )}
        </div>
                ...
                return (
                  <div key={s.label} className="flex items-center flex-1 last:flex-none">
                    <button
                      ...
                      className={cn("flex flex-col items-center gap-1 px-2", done && "cursor-pointer")}
                    >
                      <div className={cn(
                    ...
                    {i < STEPS.length - 1 && (
                      <div className={cn("h-0.5 flex-1 mx-1 transition-colors", i < step ? "bg-primary" : "bg-muted")} />
                    )}
                  </div>
                );
              })}
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-5">
                      {!hasFacility ? (
                        ...
                        <div className="space-y-5">
                          <div className="flex items-center gap-3 p-3 rounded-lg border bg-muted/20">
                            ...
                            />
                            <label htmlFor="facility-toggle" className="text-sm font-medium cursor-pointer select-none">
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <FormField control={form.control} name="sqftPerFloor" render={({ field }) => (
                                  <FormItem>
                                    ...
                                    <FormDescription>Usable floor area available for racks.</FormDescription>
                                    <FormMessage />
                                  </FormItem>
                                )} />
                                <FormField control={form.control} name="floorsUsed" render={({ field }) => (
                                  <FormItem>
                                    ...
                                    <FormDescription>Number of floors in this scenario.</FormDescription>
                                    <FormMessage />
                                  </FormItem>
                                )} />
                              </div>
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <FormField control={form.control} name="floorLevel" render={({ field }) => (
                                  <FormItem>
                                    ...
                                    <FormDescription>Building level for this deployment.</FormDescription>
                                    <FormMessage />
                                  </FormItem>
                                )} />
                                <FormField control={form.control} name="flooringType" render={({ field }) => (
                                  <FormItem>
                                    ...
                                    <FormDescription>Used to flag structural review needs.</FormDescription>
                                    <FormMessage />
                                  </FormItem>
                                )} />
                              </div>
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <FormField control={form.control} name="serverSpacingFt" render={({ field }) => (
                                  <FormItem>
                                    ...
                                    <FormDescription>Spacing between racks for airflow and access.</FormDescription>
                                    <FormMessage />
                                  </FormItem>
                                )} />
                                <FormField control={form.control} name="ceilingHeightFt" render={({ field }) => (
                                  <FormItem>
                                    ...
                                    <FormDescription>Used to identify airflow constraints.</FormDescription>
                                    <FormMessage />
                                  </FormItem>
                                )} />
                              </div>
                      ...
                      <p className="text-xs text-muted-foreground border-t pt-4">
                        Facility Constraints are for preliminary planning review and do not replace stamped engineering analysis.
                      </p>
